...
procedure TRtfdBox.PaintShadow(rounded: boolean; sw: integer; C: TCanvas;
  R: TRect);
var
  Si, i, SCol, sr, sg, sb, er, eg, eb: integer;
...
begin
  ...
  sg := GetGValue(SCol);
  sb := getBValue(SCol);
  Si := 255 - Round(FConfiguration.ShadowIntensity * 25.5);
  er := Si;
  eg := Si;
  eb := Si;
  C.Pen.Width := 2;
...
var
  R, R1: TRect;
  sw, TopH, i, Separator: integer;
  IsObject, IsClassOrInterface, IsValid: boolean;
  Pathname, S, SVGColor: string;
begin
  ...
  begin
    if Self is TRtfdClass then
      ...
      Pathname := (Self as TRtfdInterface).Pathname;
...
begin
  ...
  if FConfiguration.ShowClassparameterSeparately and (TypeParameter <> '') and
...
begin
  ...
  while Omi.HasNext do begin
    ...
    if FConfiguration.UseAbstract and Operation.Entity.IsAbstract then
  ...
  for i := 0 to ControlCount - 1 do
    ...
    else if Controls[i] is TRtfdCustomLabel then begin
      CustomLabel := TRtfdCustomLabel(Controls[i]);
      CustomLabel.SetBounds(4, aTop, Width, CustomLabel.Height);
      aTop := aTop + CustomLabel.Height;
    end
    else if Controls[i] is TRtfdSeparator then begin
...
begin
  ...
  TrMemo.Brush.color := FConfiguration.CommentColor;
  TrMemo.Parent := Self;
...
begin
  ResizeMemo; // relevant
  sw := ShadowWidth;
  TrMemo.color := FConfiguration.CommentColor;
  TrMemo.Brush.color := FConfiguration.CommentColor;
...
begin
  ...
  if not FConfiguration.ConstructorWithVisibility and (Entity is TOperation) and
  ...
  if FConfiguration.RelationshipAttributesBold and (Entity is TAttribute) and
...
function TVisibilityLabel.IntegerInsteadOfInt(const S: string): string;
begin
  Result := S;
  if FConfiguration.IntegerInsteadOfInt and (S = 'int') then
    Result := 'Integer';
end;
...
begin
  ...
  if (TypeParameter <> '') and FConfiguration.ShowClassparameterSeparately then
  begin
    // parameterized class
    ...
    Canvas.Font := Font;
    Canvas.Font.Style := [];
    FTextWidthParameter := Canvas.TextWidth(TypeAndBinding) + 12;
...
begin
  ...
  begin
    ...
    R.Right := R.Right - FExtentX - FConfiguration.ShadowWidth;
    ...
    R := Parent.ClientRect;
    R.Right := R.Right - FConfiguration.ShadowWidth - 4;
    R := Rect(R.Right - 2 * FExtentX, 0, R.Right, 2 * FExtentY);
...
begin
  ...
  case FConfiguration.ObjectCaption of
    0:
      Caption := Entity.Name + ': ' + Obj.GenericName;
    1:
...
begin
  ...
  R := ClientRect;
  R.Right := R.Right - FConfiguration.ShadowWidth;
...
begin
        ...
        case ShowParameter of
          ...
          4:
            if FConfiguration.StartWithDatatype then
              ...
              S := S + Parameter.Name + ': ' + IntegerInsteadOfInt
                (Parameter.TypeClassifier.GetShortType) + ', ';
        end
    end;
  ...
  begin
    if assigned(O.ReturnValue) then
      if FConfiguration.StartWithDatatype then
        S := IntegerInsteadOfInt(O.ReturnValue.GetShortType) + ' ' + S
      else
        S := S + ': ' + IntegerInsteadOfInt(O.ReturnValue.GetShortType)
    else if (O.OperationType <> otConstructor) and FConfiguration.UseVoid then
      if FConfiguration.StartWithDatatype then
        S := 'void ' + S
      else
        S := S + ': void';
...
begin
  ...
  begin
    ...
    begin
      if FConfiguration.StartWithDatatype then
        Caption := IntegerInsteadOfInt(A.TypeClassifier.GetShortType) +
          ' ' + A.Name
      else
        Caption := A.Name + ': ' + IntegerInsteadOfInt
          (A.TypeClassifier.GetShortType);
      if (A.Value <> '') and A.Static then
        Caption := Caption + ' = ' + A.Value;
    end;
  end
  else
    Caption := A.Name;
...
begin
  ...
  begin
    ...
    Canvas.Handle := DC;
    Canvas.Font.assign(Font);
    if FConfiguration.RelationshipAttributesBold and (Entity is TAttribute) then
