...
type
  ...
  published
    ...
    property windowActivated: string read FwindowActivated
      write FwindowActivated;
    ...
    property windowDeactivated: string read FwindowDeactivated
      write FwindowDeactivated;
    property windowDeiconified: string read FwindowDeiconified
      write FwindowDeiconified;
    property windowGainedFocus: string read FwindowGainedFocus
      write FwindowGainedFocus;
    property windowIconified: string read FwindowIconified
      write FwindowIconified;
    property windowLostFocus: string read FwindowLostFocus
      write FwindowLostFocus;
    property windowOpened: string read FwindowOpened write FwindowOpened;
    property windowStateChanged: string read FwindowStateChanged
      write FwindowStateChanged;
  end;
...
begin
  ...
  begin
    Str1 := 'int frameWidth';
    Str2 := FConfiguration.Indent2 + 'int frameWidth = ' +
      IntToStr(PPIUnScale(Width)) + '; ';
    EditForm.ReplaceLine(Str1, Str2);
    Str1 := 'int frameHeight';
    Str2 := FConfiguration.Indent2 + 'int frameHeight = ' +
      IntToStr(PPIUnScale(Height)) + ';';
    EditForm.ReplaceLine(Str1, Str2);
  end
  else if GetFrameType in [4, 7] then
  begin // Applet, JApplet
    Str1 := ' cp.setBounds(';
    Str2 := FConfiguration.Indent2 + 'cp.setBounds(0, 0, ' +
...
begin
  case ShowEvents of
    ...
    3:
      Result := FocusEvents + KeyEvents + MouseEvents + HierarchyEvents +
        InputMethodEvents + PropertyEvents + AncestorEvents1 + CaretEvents +
        ComponentEvents;
  end;
...
begin
  ...
  if MaxFontKey > 0 then
    ...
    FontSize := FConfiguration.GUIFontSize;
