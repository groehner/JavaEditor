...
begin
  DisableUpdateMenuItems;
  var UMLFenster:= MakeNewUMLWindow(getFilename('.uml'), '');
  if UMLFenster.MainModul.OpenFolderActionExecute(Sender) then begin
    UMLFenster.Pathname:= getFilename('.uml', UMLFenster.MainModul.OpendFolder);  // FOpenFolderForm.PathTreeView.Path
    UMLFenster.Save(false)
  end else
    UMLFenster.Close;
  EnableUpdateMenuItems;
end;
...
procedure TFJava.MIDiagramFromOpenFilesClick(Sender: TObject);
  var i, mr: integer; filename, path: string; DlgMessage: TDlgMessage;
      UMLForm: TFUMLForm; SL: TStringList;
begin
  ...
  try
    ...
    if mr in [mrYes, mrNone] then begin
      UMLForm:= MakeNewUMLWindow(filename, '');
