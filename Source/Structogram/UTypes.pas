...
uses
  Windows, Classes, Controls, Graphics, ExtCtrls, Forms;
...
uses SysUtils, Printers, Math, Themes, UITypes, Types,
     UUtils, UConfiguration, UMessages;
...
begin
  Result:= (SL.Count > 0) and (SL.Strings[0] <> 'JSG: true');
end;
...
begin
  with list.Canvas do begin
    ...
    w:= TextWidth(FConfiguration.No);
    TextOut(rct.left+4, h, FConfiguration.Yes);
    TextOut(rct.right-w-2, h+2, FConfiguration.No);
    Brush.Style:= bsSolid;
...
begin
  ...
  if assigned(next) and (next.prev <> Self) then
    s:= s + ' next.prev <> self';
  FMessages.OutputToTerminal(s);
  FMessages.OutputToTerminal('THEN');
  then_elem.debug;
  FMessages.OutputToTerminal('ELSE');
  else_elem.debug;
  FMessages.OutputToTerminal('IF END');
end;
...
begin
  ...
  if assigned(next) and (next.prev <> Self) then
    s:= s + ' next.prev <> self';
  FMessages.OutputToTerminal(s);
  FMessages.OutputToTerminal('DO WHILE');
  FMessages.OutputToTerminal('');
  do_elem.debug;
  FMessages.OutputToTerminal('DO WHILE END');
end;
...
begin
  ...
  if assigned(next) and (next.prev <> Self) then
    s:= s + ' next.prev <> self';
  FMessages.OutputToTerminal(s);
  FMessages.OutputToTerminal('FOR ');
  do_elem.debug;
  FMessages.OutputToTerminal('FOR END');
end;
...
begin
  ...
  for i:= 0 to high(case_elems) do begin
    FMessages.OutputToTerminal('');
    FMEssages.OutputToTerminal('CASE ' + IntToStr(i));
    case_elems[i].debug;
    FMEssages.OutputToTerminal('CASE ' + IntToStr(i) + ' END');
  end;
end;
...
begin
  ...
  Canvas:= Image.Canvas;
  SwitchWithCaseLine:= FConfiguration.SwitchWithCaseLine;
  setLineHeight;
  PuzzleMode:= Mode;
end;
...
begin
  with Canvas do begin
    ...
    if text = FConfiguration.Algorithm + ' ' then begin
...
begin
  if Kind = Byte(nsAlgorithm)
    then Sw:= FConfiguration.StructogramShadowWidth
    else Sw:= 0;
...
begin
  i:= Pos(FConfiguration.Algorithm + ' ', text);
  if i > 0
    then s:= copy(list.text, i + Length(FConfiguration.Algorithm + ' '), 255)
    else s:= text;
  i:= Pos('(', s);
  if i > 0 then
    delete(s, i, length(s));
