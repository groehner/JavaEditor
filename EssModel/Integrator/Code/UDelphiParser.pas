...
uses Classes, uCodeParser, uModel, UUtils;
...
type
  ...
  public
    ...
    procedure ParseStream(AStream: TStream; AModel: TAbstractPackage; AOM: TObjectModel;
                filename: string; inner: boolean; withoutNeedSource: boolean); override;
    procedure ParseStreamWithDefines(AStream: TStream; AModel: TAbstractPackage; AOM: TObjectModel; const GlobalDefines: TStringList; const Filename: string );
    ...
    begin
      case FCurrPos^ of
        ...
        '''' :
          begin
            Inc(FCurrPos);
            while not CharInSet(FCurrPos^, ['''', #10, #13]) or ((FCurrPos^ = '''') and ((FCurrPos + 1)^ = '''')) do
            begin
...
begin
  ...
  case FCurrPos^ of
    ...
    '#' : // Parse char
      begin
        AddOne;
        while CharInSet(FCurrPos^, ['0'..'9']) do AddOne;
      end;
    '$' : // Parse hexnum
      begin
        AddOne;
        while CharInSet(FCurrPos^, ['0'..'9', 'A'..'F', 'a'..'f']) do AddOne;
      end;
    ':' :
      begin
        AddOne;
    ...
    '+', '-', '0'..'9' : // Parse numerics
      begin
        ...
        if CharInSet((FCurrPos - 1)^, ['0'..'9']) and CharInSet(FCurrPos^, ['e', 'E']) then
        begin
          AddOne;
          if CharInSet(FCurrPos^, ['-', '+']) then AddOne;
          while CharInSet(FCurrPos^, ['0'..'9']) do AddOne;
        end;
      end;
    '<', '>' :
    ...
    'A'..'Z', 'a'..'z', '_', '@' :
      begin
        AddOne;
        while CharInSet(FCurrPos^, ['A'..'Z', 'a'..'z', '0'..'9', '_', '@']) do AddOne;
      end;
  else
    raise Exception.Create(FUnit.Name + ': Unhandled parser state character:'+FCurrPos^);
  end;
  Result := True;
end;
...
procedure TDelphiParser.ParseStreamWithDefines(AStream: TStream; AModel: TAbstractPackage;
                          AOM: TObjectModel; const GlobalDefines: TStringList; const Filename: string );
begin
...
begin
  ...
  begin
    ...
    begin
      ...
      raise Exception.Create(FUnit.Name + ': Parseprogramblock: Token:' + Token + ' Next:' + NextToken+' Context:'+Copy(FCurrPos,-100,200));
  end;
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        raise Exception.Create(FUnit.Name + ' : ParseImplementation: Token:' + Token + ' Next:' + NextToken);
    end;
  end;
end;
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        raise Exception.Create(FUnit.Name + ' : Parse Interface Section: Token:' + Token + ' Next:' + NextToken);
    end;
  end;
end;
...
begin
  if GetNextToken then begin
    while Token <> ';' do begin
      ...
      if Assigned(NeedPackage) and (FOM.ModelRoot.FindUnitPackage(uName) = nil) then begin
        str:= nil;
        NeedPackage(fName, '', str, not Recurse);
        if Assigned(str) and Recurse then begin
...
begin
  ...
  begin
    ...
    begin
      ...
      begin
        ex := FUnit.FindClassifier(tName, TClass);
        //Have to be in the current unit, it is ok to decalre a new class with the
        // same name as an existing class
        if (not Assigned(ex)) or (ex.Owner<>FUnit) then begin
          ex := FUnit.MakeClass(tName, '');
          ex.IsVisible:= true;
          FUnit.AddClass(TClass(ex));
        end;
    ...
    begin
      ex := FUnit.FindClassifier(tName, TInterface);
      if (not Assigned(ex)) or (ex.Owner<>FUnit) then begin
        ex := FUnit.MakeInterface(tName, '');
        ex.IsVisible:= true;
        FUnit.AddInterface(TInterface(ex));
      end;
      ex.Visibility := Visibility;
