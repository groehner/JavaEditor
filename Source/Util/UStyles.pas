...
type
  ...
  protected
    ...
    procedure ChangeScale(Muli, Divi: Integer; IsDpiChange: Boolean); override;
  public
...
procedure TVclStylesPreview.ChangeScale(Muli, Divi: Integer; IsDpiChange: Boolean);
begin
  FMenuHeight := MulDiv(FMenuHeight, Muli, Divi);
  FMenuCaptionWidth := MulDiv(FMenuCaptionWidth, Muli, Divi);
  FButtonWidth := MulDiv(FButtonWidth, Muli, Divi);
  FButtonHeight := MulDiv(FButtonHeight, Muli, Divi);
  FMargin := MulDiv(FMargin, Muli, Divi);
...
var
  ...
  procedure ScaleLeftContentRect(var Rect: TRect);
  begin
  ...
  procedure ScaleRightContentRect(var Rect: TRect; const OutsideR: TRect);
  begin
    Rect.Top := MulDiv(Rect.Top, DPI, 96);
    Rect.Left := OutsideR.Right - MulDiv(OutsideR.Right - Rect.Left, DPI, 96);
    Rect.Right := OutsideR.Right - MulDiv(OutsideR.Right - Rect.Right, DPI, 96);
    Rect.Bottom := MulDiv(Rect.Bottom, DPI, 96);
  end;
  ...
  function RectVCenter(var Rect: TRect; Bounds: TRect): TRect;
  begin
    OffsetRect(Rect, -Rect.Left, -Rect.Top);
    OffsetRect(Rect, 0, (Bounds.Height - Rect.Height) div 2);
...
begin
  ...
  try
    ...
    if not Style.GetElementContentRect(0, IconDetails, CaptionRect, ButtonRect)
    then
    ...
    if (ButtonRect.Width > 0) and (FIcon <> 0) then
      DrawIconEx(CaptionBitmap.Canvas.Handle, IconRect.Left, IconRect.Top,
        FIcon, IconRect.Right - IconRect.Left, IconRect.Bottom - IconRect.Top,
        0, 0, DI_NORMAL);
  ...
  LRect := Rect(BorderRect.Left, BorderRect.Top + 1,
    ARect.Width - BorderRect.Left, BorderRect.Top + 1 + FMenuHeight);
  ...
  CaptionRect := Rect(LRect.Left + FMargin, LRect.Top + 3, LRect.Right,
    LRect.Bottom);
  Style.DrawText(FBitmap.Canvas.Handle, LDetails, 'File', CaptionRect, [tfLeft],
    ThemeTextColor, DPI);
  CaptionRect := Rect(LRect.Left + FMargin + FMenuCaptionWidth, LRect.Top + 3,
    LRect.Right, LRect.Bottom);
  Style.DrawText(FBitmap.Canvas.Handle, LDetails, 'Edit', CaptionRect, [tfLeft],
    ThemeTextColor, DPI);
  CaptionRect := Rect(LRect.Left + FMargin + FMenuCaptionWidth * 2,
    LRect.Top + 3, LRect.Right, LRect.Bottom);
  Style.DrawText(FBitmap.Canvas.Handle, LDetails, 'View', CaptionRect, [tfLeft],
    ThemeTextColor, DPI);
  CaptionRect := Rect(LRect.Left + FMargin + FMenuCaptionWidth * 3,
    LRect.Top + 3, LRect.Right, LRect.Bottom);
  Style.DrawText(FBitmap.Canvas.Handle, LDetails, 'Help', CaptionRect, [tfLeft],
    ThemeTextColor, DPI);
  ...
  begin
    ...
    ButtonRect.Left := BorderRect.Left + FMargin div 2 + (I - 1) *
      FButtonWidth;
    ...
    Style.GetElementColor(LDetails, ecTextColor, ThemeTextColor);
    Style.DrawText(FBitmap.Canvas.Handle, LDetails,
      'ToolButton' + IntToStr(I), ButtonRect,
