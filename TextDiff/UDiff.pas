  ...
  try
    ...
    for i := 1 to count -1 do
      with PCompareRec(fCompareList[i])^ do
        if (Kind = ckModify) and (int1 = int2) then begin
          Kind := ckNone;
...
begin
  var idx := fDiffList.Count -1;
  Result := idx >= 0;
  if not result then exit;
  var DiffVars := PDiffVars(fDiffList[idx]);
  with DiffVars^ do
    if fCompareInts
...
begin
  ...
  while (fLastCompareRec.oldIndex1 < offset1 -1) do begin
