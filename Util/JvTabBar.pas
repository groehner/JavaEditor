...
G. Röhner: DPI-Awareness via PPIScale
-----------------------------------------------------------------------------}
// $Id$
...
uses
  Windows, Messages, Vcl.Menus, Types, Graphics, Controls, Forms, ImgList,
  Buttons, Themes, JvThemes, ExtCtrls, SysUtils, Classes, Contnrs, System.UITypes;
...
procedure TJvTabBarItem.SetColor(const Value: TColor);
begin
...
begin
  ...
  begin
    ...
    begin
      ...
      LineTo(CloseR.Left + dx + q, CloseR.Top + dy + q);
      MoveTo(CloseR.Left + dx + q, CloseR.Top + dy);
      LineTo(CloseR.Left + dx, CloseR.Top + dy + q);
...
begin
  Result.Left := R.Left + (Owner as TForm).PPIScale(5);
  Result.Top :=  R.Top + (Owner as TForm).PPIScale(5);
  Result.Right := Result.Left + (Owner as TForm).PPIScale(12);
  Result.Bottom := Result.Top + (Owner as TForm).PPIScale(11);
end;
...
begin
  ...
  Result.cx := Canvas.TextWidth(Tab.Caption) + Tab.TabBar.PPIScale(11);
  Result.cy := Canvas.TextHeight(Tab.Caption + 'Ag') + Tab.TabBar.PPIScale(7);
  if Tab.TabBar.CloseButton then
    Result.cx := Result.cx + Tab.TabBar.PPIScale(15);
  if (Tab.ImageIndex <> -1) and (Tab.GetImages <> nil) then
    Result.cx := Result.cx + Tab.GetImages.Width + Tab.TabBar.PPIScale(2);
