...
begin
  ...
  try
    ...
    True, // bInheritHandles
    // dwCreationFlags
    CREATE_NEW_CONSOLE or NORMAL_PRIORITY_CLASS, nil, // pEnvironment
    ...
    then
      ...
      ErrorMsg(Format(_(LNGUnabledToExecute), [Batchfile]) + ' ' +
        SysErrorMessage(GetLastError));
  finally
    TerminateTask(FProcessInformation);
    FProcessRunning := False;
...
function TJavaCommands.ShellExecuteFile(const FileName, Params,
...
begin
  ...
  Call := '-jar ' + HideBlanks(FConfiguration.JUnitJarFile) + ' -cp ' +
    Classpath + ' ' + FConfiguration.JUnitParameter +
    ' --disable-ansi-colors --details=tree';
  if Method <> 'Class' then
    Call := Call + ' -m ' + AClassname + #4 + Method
  else
    Call := Call + ' -c ' + AClassname;

  FMessages.LBMessages.Clear;
  FMessages.OutputLineTo(K_Messages, FConfiguration.JavaInterpreter +
    ' ' + Call);
  if ExecAndWait(FConfiguration.JavaInterpreter, Call,
    ExtractFilePath(AClass.Pathname), FErrFile, SW_HIDE) then
  begin
    ...
    begin
      ...
      begin
        ...
        while (Posi = 0) and (Idx < StringList.Count - 1) do
        begin
          Inc(Idx);
          Posi := Pos('-- ' + AMethod.Name, StringList[Idx]);
