...
begin
  ...
  begin
    ...
    Width := TextWidth(FConfiguration.No_);
    TextOut(FRct.Left + 4, Height + 6, FConfiguration.Yes);
    TextOut(FRct.Right - Width - 2, Height + 6, FConfiguration.No_);
    Brush.Style := bsSolid;
...
var
  Str: string;
begin
  Str := 'TStrWhile FKind = ' + IntToStr(FKind) + ' Text= ' + FText + ' Rct(' +
    IntToStr(FRct.Left) + ', ' + IntToStr(FRct.Top) + ', ' +
    IntToStr(FRct.Width) + ', ' + IntToStr(FRct.Bottom) + ')';
  if Assigned(FPrev) and (FPrev.FNext <> Self) then
    Str := Str + ' Prev.Next <> self';
...
begin
  inherited Create(List);
  FKind := Ord(nsSwitch);
  SetLength(FCaseElems, FConfiguration.CaseCount);
...
begin
  ...
  begin
    ...
    if FText = FConfiguration.Algorithm + ' ' then
    begin
...
begin
  ...
  Reader.ReadLine; // Algorithm or FList head
  FText := Reader.FText;
  FRct := Reader.FARect;
  FTextPos := Reader.FPoint;
  Current := Self;
...
begin
  if FKind = Byte(nsAlgorithm) then
    ShadowWidth := FConfiguration.StructogramShadowWidth
  else
    ShadowWidth := 0;
...
var
  ...
  Str: string;
begin
  Inte := Pos(FConfiguration.Algorithm + ' ', FText);
  if Inte > 0 then
    Str := Copy(FList.FText, Inte + Length(FConfiguration.Algorithm + ' '), 255)
  else
    Str := FText;
  Inte := Pos('(', Str);
  if Inte > 0 then
