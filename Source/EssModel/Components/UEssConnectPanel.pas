...
{ Thoughts on the representation in the UML window
  ...
  The background does not have to be emptied here,
  it is sufficient if the boxes are redrawn.
  If two boxes overlap, the box you clicked on must be brought to the front.
  Therefore, when clicking, the entire box must be drawn in any case,
  it is not enough to just draw the markings.
}
...
type
  ...
  private
    ...
    FPopupMenuAlign: TSpTBXPopupMenu;
    FPopupMenuConnection: TSpTBXPopupMenu;
    FPopupMenuWindow: TSpTBXPopupMenu;
  ...
  protected
    ...
    procedure MouseMove(Shift: TShiftState; XPos, YPos: Integer); override;
  ...
  public
    ...
    procedure TextTo(Canvas: TCanvas);
    ...
    property OnDeleteSelectedControls: TNotifyEvent
      read FOnDeleteSelectedControls write FOnDeleteSelectedControls;
    property OnClassEditSelectedDiagramElements: TNotifyEvent
      read FOnClassEditSelectedDiagramElements
      write FOnClassEditSelectedDiagramElements;
    property OnFormMouseDown: TNotifyEvent read FOnFormMouseDown
      write FOnFormMouseDown;
    property PopupMenuAlign: TSpTBXPopupMenu read FPopupMenuAlign
      write FPopupMenuAlign;
    property PopupMenuConnection: TSpTBXPopupMenu read FPopupMenuConnection
      write FPopupMenuConnection;
    property PopupMenuWindow: TSpTBXPopupMenu read FPopupMenuWindow
      write FPopupMenuWindow;
    // Only draw selected
...
procedure TEssConnectPanel.DrawMarkers(Rec: TRect; Show: Boolean);
  ...
  var
    ARect: TRect;
  begin
    ARect := Rect(X, Y, X + HANDLESIZE, Y + HANDLESIZE);
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        if (Conn1.FromControl = Conn2.FromControl) and
          (Conn1.ToControl = Conn2.ToControl) or
          (Conn1.FromControl = Conn2.ToControl) and
          (Conn1.ToControl = Conn2.FromControl) then
        begin
...
begin
  ...
  begin
    ...
    if FConfiguration.RoleHidesAttribute then
...
begin
  ...
  begin
    ...
    if (Conn.FromControl is TRtfdObject) and (Conn.ToControl is TRtfdObject)
    then
...
procedure TEssConnectPanel.SetRecursiv(Posi: TPoint; Pos: Integer);
begin
  Posi := Self.ScreenToClient(Posi);
  var
  Num := 0;
  while Num < FConnections.Count do
  begin
    var
    Conn := TConnection(FConnections[Num]);
    if Conn.IsClicked(Posi) then
...
begin
  var
  FAssociation := TFAssociationDialog.Create(Self);
  try
    var
...
procedure TEssConnectPanel.MouseMove(Shift: TShiftState; XPos, YPos: Integer);
var
  Point, Point1, Point2: TPoint;
  Found, Src: TControl;
  MCont: TManagedObject;
  DeltaX, DeltaY, SelControls, HandleShadow, Newleft, NewTop, AWidth,
    AHeight: Integer;
  Curr: TCrackControl;
  ...
  begin
    ...
    begin
      ...
      Point2.X := Point2.X + Mdx;
      Point2.Y := Point2.Y + Mdy;
      Mouse.CursorPos := Point2;
      Resized := True;
    end;
  end;
  ...
  begin
    ...
    begin
      ...
      if Connect.Visible and Connect.ToControl.Visible and
        Connect.FromControl.Visible and
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        HandleShadow := Max(HANDLESIZE - FConfiguration.ShadowWidth, 0);
          ...
          begin
            ...
            begin
              ...
              begin
                ...
                NewTop := Curr.Top;
                if (DeltaX <> 0) and
                  (Curr.Left + DeltaX >= GetRecursivXDecrease(MCont.FControl))
                then
                  Newleft := Curr.Left + DeltaX;
                if (DeltaY <> 0) and
                  (Curr.Top + DeltaY >= GetRecursivYDecrease(MCont.FControl))
                then
...
begin
  if Assigned(FManagedObjects) then
    ...
    begin
      var
      AControl := TManagedObject(FManagedObjects[I]).FControl;
      if (AControl is TRtfdClass) or (AControl is TRtfdInterface) or
        (AControl is TRtfdCommentBox) then
      begin
...
procedure TEssConnectPanel.TextTo(Canvas: TCanvas);
  ...
  if Assigned(FManagedObjects) then
    ...
    begin
      ...
      begin
        ...
        DrawTextEx(Canvas.Handle, Pointer(Str), Length(Str), Rect,
