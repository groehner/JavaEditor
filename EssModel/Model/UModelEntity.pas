...
uses Sysutils, Windows, StrUtils, uListeners;
...
begin
  ...
  Dec(DocN);
  try
    FreeAndNil(Listeners);   // here we get a memory leak
  except
  end;
  inherited;
end;
...
begin
  ...
  try
    ...
    var i:= LastDelimiter('.', withoutGeneric(Value));
    if i > 0
