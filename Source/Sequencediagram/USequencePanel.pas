...
type
  ...
  public
    ...
    function ArrowStartLength(Arrow: TArrowStyle): integer;
    function ArrowHeadLength(Arrow: TArrowStyle): integer;
  ...
  public
    ...
    function HaveConnection(Src, Dest: TControl; ArrowStyle: TArrowStyle): integer; overload;
...
function TConnection.ArrowHeadLength(Arrow: TArrowStyle): integer;
begin
  if arrow in [casAsynchron, casReturn]
    then Result:= HeadLength
    else Result:= 0;
end;

function TConnection.ArrowStartLength(Arrow: TArrowStyle): integer;
begin
  if arrow = casReturn
    then Result:= HeadLength
    else Result:= 0
...
procedure TSequencePanel.ShowAllIntersecting(R: TRect);
  var i: Integer; R1: TRect;
...
begin
  for i:= 0 to FManagedObjects.Count -1 do begin
    R1:= TManagedObject(FManagedObjects[i]).FControl.BoundsRect;
    if not Intersect(R, R1) or not TManagedObject(FManagedObjects[i]).Visible then
      // SuspendDrawing
      SendMessage((TManagedObject(FManagedObjects[i]).FControl as TWinControl).Handle , WM_SETREDRAW, 0, 0)
    else
      TManagedObject(FManagedObjects[i]).FControl.Invalidate;
  end;
...
function TSequencePanel.getRecursivConnection(Src: TControl): TConnection;
begin
  var i:= HaveConnection(Src, Src);
  if i > -1
...
procedure TSequencePanel.setRecursiv(P: TPoint; pos: integer);
begin
...
begin
  with TFConnectDialog.Create(Self) do begin
...
procedure TSequencePanel.ConnectBoxes(Src, Dest: TControl);
begin
  with TFConnectDialog.Create(Self) do begin
    ...
    if ShowModal =  mrOK then begin
      var Attributes:= getConnectionAttributes;
      case SelectedControls of
...
procedure TSequencePanel.SetConnection(i: Integer; Attributes: TConnectionAttributes);
begin
...
function TSequencePanel.HaveConnection(Src, Dest: TControl; ArrowStyle: TArrowStyle): integer;
begin
  for var i:= 0 to FConnections.Count-1 do begin
    var Conn:= TConnection(FConnections[i]);
...
begin
  var mdy := TScrollBox(Parent).VertScrollBar.Position - WheelDelta;
...
begin
  var inst:= FindManagedControl(Sender as TControl);
...
begin
  ...
  if CanFocus and Assigned(F) and F.Active then begin
...
procedure TSequencePanel.SetSelectedOnly(const Value : boolean);
  var i: integer;
begin
  if FSelectedOnly <> Value then begin
    FSelectedOnly := Value;
    if FSelectedOnly then begin
      TempHidden.Clear;
      for i:= 0 to FManagedObjects.Count -1 do
...
function TSequencePanel.getClipRect: TRect;
