...
type
  ...
  private
    ...
    FControl: TControl;
    FName: string; // handle UML-objects special
    FNameNr: Integer;
  public
    constructor Create;
...
begin
  ...
  try
    ...
    on e: Exception do
      FConfiguration.Log('TSugiyamaLayout.Execute', e);
  end;
end;

// Extract FNodes FromControl essconnectpanel
...
begin
  ...
  try
    ...
    begin
      ...
      begin
        ...
        if (Posi > 0) and IsDigit(Node.FName[Posi]) then
        begin
          while IsDigit(Node.FName[Posi]) do
            Dec(Posi);
          Node.FNameNr := StrToInt(Copy(Node.FName, Posi + 1,
            Length(Node.FName)));
          Delete(Node.FName, Posi + 1, Length(Node.FName));
        end
        else
          Node.FNameNr := 0;
      end;
  ...
  try
    ...
    begin
      ...
      // Here the connection is reversed: FromControl=to, to=FromControl
...
begin
  ...
  try
    ...
    begin
      ...
      begin
        // If all FLayers has LayerMaxNodes nr of FNodes, then create a new layer
        FLayers.Add(TNodeList.Create(False));
...
var
  ...
  var
    FToNode: TNode;
  begin
    FToNode := Node.FOutEdges[EPos].FToNode;
    for var I := 0 to FToNode.FInEdges.Count - 1 do
      if FToNode.FInEdges[I].FFromNode = Node then
      begin
        FToNode.FInEdges.Delete(I);
        Node.FOutEdges.Delete(EPos);
        AddEdge(FToNode, Node);
        Break;
      end;
  end;
      ...
      begin // Real cycle found
        ...
        begin // Find edge with min(outdeg(VNode)) max(indeg(VNode) + indeg(WNode))
          ...
          for var J := 0 to VNode.FOutEdges.Count - 1 do
            ...
            begin
              Score := VNode.FInEdges.Count + VNode.FOutEdges[J]
                .FToNode.FInEdges.Count - VNode.FOutEdges.Count;
...

{
  ...
  Topological FOrder:
  A numbering of the FNodes of a directed acyclic graph such that every edge FromControl a node
    ...
    begin
      ...
      begin
        ...
        begin
          ...
          if (Indeg[Edge.FToNode.FId] = 0) and (GLabels[Edge.FToNode.FId] = 0)
          then
...
begin
  ...
  begin
    // Put all FNodes in a layer with the same Y, increase Y with highest node + spacing
    Highest := 0;
    ...
    begin
      ...
      begin
        ...
        while (JPos < Layer.Count) and (LastForce >= Forces[JPos]) and
          (Layer[JPos].FXPos - (Layer[JPos - 1].FXPos + Layer[JPos - 1].FWidth) <=
          FHSpacing) do
        begin
          ...
          begin
            ...
            if RegStart = 0 then
              ...
              MaxAmount := Layer[RegStart].FXPos -
                (Layer[RegStart - 1].FXPos + Layer[RegStart - 1].FWidth +
                FHSpacing);
          ...
          begin
            ...
            if RegStart + RegCount = Layer.Count then
              ...
              MaxAmount := Layer[RegStart + RegCount].FXPos -
                (Layer[RegStart + RegCount - 1].FXPos +
                Layer[RegStart + RegCount - 1].FWidth + FHSpacing);
...
begin
  ...
  begin
    ...
    begin
      if TNode(Item1).FName < TNode(Item2).FName then
        Result := -1
      else if TNode(Item1).FName = TNode(Item2).FName then
      begin
        if TNode(Item1).FNameNr < TNode(Item2).FNameNr then
          Result := -1
        else if TNode(Item1).FNameNr = TNode(Item2).FNameNr then
          Result := 0
        else
          Result := 1;
      end
  ...
  try
    ...
    begin
      ...
      if not Assigned(Node) then
        Continue;
      if Odd(I) then
        Node.FOutEdges.Sort(ToNodeCOrderSortProc)
      else
        Node.FInEdges.Sort(FromNodeCOrderSortProc);
    end;
