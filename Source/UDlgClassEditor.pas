...
type
  ...
  private
    ...
    function CreateMethod(const GetSet: string;
    ...
    function MakeIdentifier(ComboBox: TComboBox; Typ: Boolean)
      : Boolean; overload;
  ...
  public
    ...
    procedure UpdateTreeView(HasChanged: Boolean = True;
      NodeIndex: Integer = -1);
...
begin
  var
  Node := TreeView.Items.AddChild(FAttributeNode, Attribute.ToNameTypeValue);
...
begin
  var
  Node := TreeView.Items.AddChild(FMethodNode, Method.ToNameParameterTyp);
...
var
  CNumberStr, Indent1, Indent2, Str: string;
  Line: Integer;
begin
  FMakeNewClass := False;
  Indent1 := GetIndent(0);
  ...
  if BInterface.Caption = _(LNGClass) then
    Str := FConfiguration.HeadText(Indent1) + CrLf + Indent1 + 'interface ' +
      EClass.Text
  else
    Str := FConfiguration.HeadText(Indent1) + CrLf + Indent1 + 'class ' +
      EClass.Text;
  if EExtends.Text <> '' then
    Str := Str + ' extends ' + EExtends.Text;
  if EImplements.Text <> '' then
    Str := Str + ' implements ' + EImplements.Text;
  Str := Str + ' {' + CrLf + Indent2 + CrLf + Indent2 + _(LNGStartGUIVariables)
    + CNumberStr + CrLf + Indent2 + _(LNGEndGUIVariables) + CNumberStr + CrLf +
    Indent2 + CrLf + Indent2 + _(LNGStartEventMethods) + CNumberStr + CrLf +
    Indent2 + _(LNGEndEventMethods) + CNumberStr + CrLf + Indent1 + '}';
...
begin
  ...
  begin
    ...
    begin
      ...
      if Abstrakt and not CBClassAbstract.Checked then
      begin
        Posi := Pos('abstract', Str);
        if Posi > 0 then
          Delete(Str, Posi, 9);
      end
      else if not Abstrakt and CBClassAbstract.Checked then
      begin
      ...
      if EExtends.Text <> '' then
        if FIsClass then
          Str := Str + ' extends ' + GetFirstWord(EExtends.Text)
        else
        begin
      ...
      if (OldClassname <> AClassname) then
      begin
        OldFile := Pathname;
        NewFile := ExtractFilePath(Pathname) + WithoutGeneric(AClassname)
          + '.java';
        ...
        else if not FileExists(NewFile) or (FMyEditor.Pathname = NewFile) or
          FileExists(NewFile) and
        ...
        begin
          ...
          FMyEditor.ReplaceWord(OldClassname,
            WithoutGeneric(NewClassname), True);
        end;
      end;
...
begin
  ...
  begin
    ...
    begin
      ...
      if BInterface.Caption = _(LNGClass) then
        ReplaceText('interface ' + AClassname, 'class ' + AClassname, True)
      else
        ReplaceText('class ' + AClassname, 'interface ' + AClassname, True);
      Modified := True;
      UpdateTreeView;
      if NodeIndex < TreeView.Items.Count then
        TreeView.Selected := TreeView.Items[NodeIndex];
...
begin
  ...
  if GetSet = _(LNGGet) then
    MethodName := _(LNGGet) + AName
  else
    MethodName := _(LNGSet) + AName;
...
begin
  if Enable and not FMyEditor.Editor.ReadOnly then
  begin
...
begin
  ...
  while ClassIterator.HasNext do
  begin
    var
...
begin
  ...
  if Assigned(Result) then
    while Assigned(Result) and (Result.ImageIndex <> 1) and
      (Result.ImageIndex <> 11) do
      Result := Result.Parent
  else
    for var I := 0 to TreeView.Items.Count - 1 do
      if TreeView.Items[I].ImageIndex in [1, 11] then
      begin
...
begin
  ...
  if Assigned(Result) then
    ...
    for var I := 0 to TreeView.Items.Count - 1 do
      if TreeView.Items[I].ImageIndex = 1 then
      begin
        Result := TreeView.Items[I];
        Break;
      end;
  ...
  try
    ...
    begin
      ...
      begin
        ...
        var
        LenE := Length(EExtends.Text);
        var
        Len := Length(Str);
        EExtends.Text := Str;
        if Len > LenE then
          EExtends.SelStart := Posi + 1
        else if Len < LenE then
          EExtends.SelStart := Posi - 1;
      end;
    ...
    begin
      ...
      begin
        ...
        else if AAttribute.Visibility = viPackage then
      ...
      begin
        ...
        if FConfiguration.AttributesAParametersP then
        begin
    ...
    begin
      ...
      begin
        ...
        if Assigned(AMethod.ReturnValue) then
          CBMethodType.Text := AMethod.ReturnValue.GetShortType
        else
          CBMethodType.Text := '';
        if FIsClass then
        begin
        ...
        begin
          ...
          if AMethod.Visibility = viPackage then
            RGMethodAccess.ItemIndex := 0
          else
            RGMethodAccess.ItemIndex := 1;
        end;
      ...
      begin
        ...
        LBParams.Clear;
        if FIsClass and (RGMethodKind.ItemIndex > 0) or not FIsClass then
        begin
    ...
    if IsAttributesNode(Node) or IsMethodsNode(Node) then
      if FMyEditor.Editor.ReadOnly then
        Line := -1
      else
      begin
        ClassNumber := GetClassNumber(Node);
        if Node.ImageIndex = 12 then
          FMyEditor.Go_To(FMyEditor.GetLNG(1, ClassNumber))
        else
          FMyEditor.Go_To(FMyEditor.GetLNG(5, ClassNumber));
...
begin
  ...
  begin
    var
    Posi := E.SelStart;
    if (Pos('"', Value) > 0) and (Pos('"', E.Text) = 0) then
      Inc(Posi);
    E.Text := Value;
    E.SelStart := Posi;
  end;
...
begin
  ...
  var
  Str := CBParamName.Text + ': ' + CBParamType.Text;
  var
  Index := ListBox.Items.IndexOf(Str);
  if Index <> -1 then
  ...
  case PageControl.ActivePageIndex of
    1:
      begin
        ...
        if FConfiguration.DefaultModifiers then
        begin
          RGAttributeAccess.ItemIndex := 0;
    ...
    2:
      begin
        ...
        CBMethodStatic.Checked := False;
        CBMethodAbstract.Checked := False;
        if FConfiguration.DefaultModifiers and FIsClass then
        begin
...
procedure TFClassEditor.AttributeToJava(Attribute: TAttribute;
  ClassNumber: Integer);
var
  Method: TOperation;
begin
  ...
  if FIsClass and CBGetMethod.Checked and not HasMethod(_(LNGGet), Attribute,
    Method) then
    FMyEditor.InsertProcedure(ClassNumber, CreateMethod(_(LNGGet), Attribute));
  if FIsClass and CBSetMethod.Checked and not HasMethod(_(LNGSet), Attribute,
    Method) then
  ...
  begin
    if CBGetMethod.Checked then
      if Assigned(Method1) then
        FMyEditor.ReplaceMethod(Method1, NewGet)
      else
        FMyEditor.InsertProcedure(ClassNumber, NewGet)
  ...
  begin
    if CBSetMethod.Checked then
      if Assigned(Method2) then
        FMyEditor.ReplaceMethod(Method2, NewSet)
      else
        FMyEditor.InsertProcedure(ClassNumber, NewSet)
...
begin
  Node := TreeView.Selected;
  if not(MakeIdentifier(CBAttributeType, True) and
    MakeIdentifier(EAttributeName, False)) or (EAttributeName.Text = '') or
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        try
          StringList.Text := FMyEditor.GetSource(Method.LineS,
            Method.LineE - 2);
          for Posi := StringList.Count - 1 downto 0 do
          begin
            var
            Str := Trim(StringList[Posi]);
      ...
      if HasSourceCode and
        ...
        #13 + _('Delete method with sourcecode?'), mtConfirmation,
...
function TFClassEditor.CreateMethod(const GetSet: string;
  const Attribute: TAttribute): string;
var
  Str, AName, Attname, BigName, Indent1, Indent2, Datatype, Param: string;
begin
  ...
  if FConfiguration.AttributesAParametersP and (Copy(Param, 1, 1) = 'a') then
  begin
    Param[1] := 'p';
    Delete(AName, 1, 1);
  end;
  if FConfiguration.SetterWithoutThis then
    Param := Param + _('New')
  else
  ...
  if GetSet = _(LNGGet) then
    Str := Indent1 + 'public ' + Attribute.TypeClassifier.GetShortType + ' ' +
      _(LNGGet) + BigName + '() {' + CrLf + Indent2 + 'return ' + Attribute.Name
      + ';' + CrLf + Indent1 + '}' + CrLf + CrLf
  else
    Str := Indent1 + 'public void ' + _(LNGSet) + BigName + '(' + Datatype + ' '
      + Param + ') {' + CrLf + Indent2 + Attname + ' = ' + Param + ';' + CrLf +
      Indent1 + '}' + CrLf + CrLf;
  Result := Str;
end;
...
var
  Str, Str1, Typ, AName, Val, Indent1, Indent2: string;
...
begin
  ...
  begin // iterate over all attributes
    ...
    begin // exists a corresponding parameter?
      ...
      begin
        var
        Par := Parameter.Name + ': ' + Parameter.TypeClassifier.GetShortType;
    ...
    begin // corresponding parameter exists or
      Str := Str + Indent2 + 'this.' + Parameter.Name + ' = ' + Parameter.Name +
        ';' + CrLf;
      DeleteSource('this.' + Parameter.Name);
      Parameter.UsedForAttribute := True;
    end
    else if (Pos('=', Node.Text) > 0) then
    begin
      var
      Str3 := Copy(Node.Text, 1, Pos(':', Node.Text) - 1);
  ...
  begin
    ...
    if (Pos('DE_LE_TE', Str1) = 0) and (Pos(Str1, Str) = 0) and
      (Pos(' return ', Str1) = 0) then
      Str := Str + Str1;
  end;
...
begin
  ...
  while (Length(Str1) > 0) and not(Str1[1].IsLetter or (Str1[1] = '_')) do
  begin
    Delete(Str1, 1, 1);
    Result := 1;
  end;
  ...
  while Int <= Length(Str1) do
    if not(Str1[Int].IsLetterOrDigit or (Pos(Str1[Int], Chars) > 0)) then
    begin
      Delete(Str1, Int, 1);
      Result := Int;
    end
  ...
  Int1 := Pos('[', Str2);
  Int2 := Pos(']', Str2);
  if (Int1 > 0) and ((Int2 = 0) or (Int2 < Int1)) or ((Int1 = 0) and (Int2 > 0))
  then
    Result := -1;
  ...
  if (Str3 = 'final') or (Str3 = 'static') or (Str3 = 'public') or
    (Str3 = 'private') then
    Result := -1;
end;

function TFClassEditor.MakeIdentifier(ComboBox: TComboBox;
  Typ: Boolean): Boolean;
var
  ErrPos, Len: Integer;
  Str: string;
...
begin
  ...
  Str := ComboBox.Text;
  var
  Index := ComboBox.Items.IndexOf(Str);
  if Index <> -1 then
...
begin
  ...
  if FIsClass then
    ...
    Method.OperationType := otProcedure
  else
    Method.OperationType := otFunction;
  if FIsClass then
    Method.Visibility := TVisibility(RGMethodAccess.ItemIndex)
  else if RGMethodAccess.ItemIndex = 0 then
    Method.Visibility := viPackage
  else
    Method.Visibility := viPublic;
...
procedure TFClassEditor.SetClassOrInterface(IsClass: Boolean);
  ...
  begin
    if RadioGroup.Items.Text <> Text then
    begin
      var
...
procedure TFClassEditor.UpdateTreeView(HasChanged: Boolean = True;
  NodeIndex: Integer = -1);
var
...
function TFClassEditor.CreateTreeView(Editor: TFEditForm;
...
begin
  if (CBParamName.Text = '') or (CBParamType.Text = '') then
    Exit;
...
begin
  ...
  begin
    ...
    if EAttributeValue.CanFocus then
      EAttributeValue.SetFocus;
  end;
end;
...
begin
  ...
  if (Posi = -1) or (CBAttributeType.Text <> CBAttributeType.Items[Posi]) then
    Str := CBAttributeType.Text
  else
    Str := CBAttributeType.Items[Posi];
  if not FComboBoxInvalid then
  begin
...
procedure TFClassEditor.CBMethodnameKeyUp(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
  if (CBMethodType.Text <> '') or (FIsClass and (RGMethodKind.ItemIndex = 1) or
...
begin
  ...
  if (Posi = -1) or (CBMethodName.Text <> CBMethodName.Items[Posi]) then
    Str := CBMethodName.Text
  else
    Str := CBMethodName.Items[Posi];
  if not FComboBoxInvalid then
  begin
...
begin
  ...
  if (Posi = -1) or (CBMethodType.Text <> CBMethodType.Items[Posi]) then
    Str := CBMethodType.Text
  else
    Str := CBMethodType.Items[Posi];
  BMethodApply.Enabled := True;
...
begin
  ...
  if (Str <> _(LNGSelectAttributes)) and (LBParams.Items.IndexOf(Str) = -1) then
  begin
...
begin
  ...
  if (Posi = -1) or (CBParamType.Text <> CBParamType.Items[Posi]) then
    Str := CBParamType.Text
  else
    Str := CBParamType.Items[Posi];
  if not FComboBoxInvalid then
  begin
...
begin
  if FIsClass and (RGMethodKind.ItemIndex = 0) then
  begin
  ...
  CBMethodName.Enabled := FIsClass and (RGMethodKind.ItemIndex > 0) or
    not FIsClass;
  ...
  case PageControl.ActivePageIndex of
    1:
      begin
        ...
        if CBAttributeType.Text = '' then
          CBAttributeType.Color := clInfoBk
        else
          CBAttributeType.Color := clWindow;
        if FIsClass then
          EAttributeValue.Color := clWindow
        else if EAttributeValue.Text = '' then
          EAttributeValue.Color := clInfoBk
        else
          EAttributeValue.Color := clWindow;
      end;
    2:
      begin
        ...
        if CBMethodType.Enabled and (CBMethodType.Text = '') and
          (FIsClass and (RGMethodKind.ItemIndex = 2) or not FIsClass and
          (RGMethodKind.ItemIndex = 1)) then
...
var
  Str, Str1, Indent1, Indent2: string;
  StringList: TStringList;
...
begin
  ...
  begin
    ...
    case Method.OperationType of
      otConstructor:
        Str := MakeConstructor(Method, Source);
      otFunction:
        begin
          var
          Str2 := ' return ' + Typ2Value(Method.ReturnValue.GetShortType) + ';';
          ...
          begin
            ...
            Str := Str + Indent2 + _(LNGTODO) + CrLf + Indent2 + Str2 +
              CrLf + Str1;
        end;
      otProcedure:
        begin
          ...
          begin
            ...
            begin
              var
              Pos1 := Pos(CrLf, Source);
              var
              Str3 := Copy(Source, 1, Pos1 + 1);
            ...
            Str := Str + Indent2 + _(LNGTODO) + CrLf + Indent2 + CrLf + Str1;
        end;
    end;
  end;
  LBParams.Items.Clear;
...
begin
  ...
  CBMethodType.Enabled := (RGMethodKind.ItemIndex = 2) or
    (not FIsClass and (RGMethodKind.ItemIndex = 1));
  if not(MakeIdentifier(CBMethodName, False) and MakeIdentifier(CBMethodType,
    True) and MakeIdentifier(CBParamName, False) and MakeIdentifier(CBParamType,
    True)) or not Assigned(Node) then
    Exit;
  RGMethodKindChange;
  if FIsClass then
    Index := 2
    ...
    Index := 1;
  if (CBMethodName.Text = '') or ((RGMethodKind.ItemIndex = Index) and
    (CBMethodType.Text = '')) or
    ((CBParamName.Text <> '') and (CBParamType.Text = '')) then
    Exit;
  ClassNumber := GetClassNumber(Node);
  NodeIndex := Node.AbsoluteIndex;
  TopItemIndex := TreeView.TopItem.AbsoluteIndex;
  ...
  begin
    if (RGMethodKind.ItemIndex = 0) and FIsClass then
    begin
      Inc(NodeIndex);
      Node := Node.GetNext;
      while Assigned(Node) and (Node.ImageIndex = 6) do
      begin
        Inc(NodeIndex);
        Node := Node.GetNext;
      end;
    ...
    if not MethodAlreadyExists(Method.ToNameParameterTyp) then
    begin
      New := MethodToJava(Method, '');
      if FIsClass and (RGMethodKind.ItemIndex = 0) then
        FMyEditor.InsertConstructor(ClassNumber, New)
      else
        FMyEditor.InsertProcedure(ClassNumber, New);
    end
  ...
  begin
    ...
    begin
      ChangeMethod(Method);
      if Method.HasSourceCode then
        Source := FMyEditor.GetSource(Method.LineS, Method.LineE - 2)
      else
        Source := '';
      if Method.OperationType = otConstructor then
        New := MakeConstructor(Method, Source)
      else
        New := MethodToJava(Method, Source);
...
begin
  FConfiguration.WriteIntegerU('ClassEditor', 'RGAttributeAccess',
  ...
  FConfiguration.WriteIntegerU('ClassEditor', 'RGMethodKind',
    RGMethodKind.ItemIndex);
  FConfiguration.WriteIntegerU('ClassEditor', 'RGMethodAccess',
    RGMethodAccess.ItemIndex);
  FConfiguration.WriteBoolU('ClassEditor', 'CBMethodStatic',
    CBMethodStatic.Checked);
  FConfiguration.WriteBoolU('ClassEditor', 'CBMethodAbstract',
    CBMethodAbstract.Checked);
end;
...
begin
  ...
  begin
    RGAttributeAccess.ItemIndex := FConfiguration.ReadIntegerU('ClassEditor',
      'RGAttributeAccess', 0);
    CBGetMethod.Checked := FConfiguration.ReadBoolU('ClassEditor',
      'CBGetMethod', False);
    CBSetMethod.Checked := FConfiguration.ReadBoolU('ClassEditor',
      'CBSetMethod', False);
    RGMethodKind.ItemIndex := FConfiguration.ReadIntegerU('ClassEditor',
      'RGMethodKind', 1);
    RGMethodAccess.ItemIndex := FConfiguration.ReadIntegerU('ClassEditor',
      'RGMethodAccess', 3);
  end;
  EnableEvents(True);
  ChangeStyle;
...
var
  From, To_, After, After_To, NodeIndex: Integer;
  EmptyLines: string;
...
begin
  if IsClassOrInterface(SourceNode) or IsAttributesNode(SourceNode) or
    ...
    FMyEditor.Editor.ReadOnly then
    Exit;
  ...
  if IsMethodsNodeLeaf(SourceNode) and
    (Trim(FMyEditor.Editor.LineS[After_To]) = '') then
    Inc(After_To);
...
function TFClassEditor.AttributeAlreadyExists(const AttributeName
  : string): Boolean;
begin
  var
  Cent := GetClassifier(GetClassNode);
